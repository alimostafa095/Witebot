...
# define ${var.az_count} private subnets To REDIS (one for each AZ)
resource "aws_subnet" "private_redis_subnet" {
  ...
  map_public_ip_on_launch = false
  tags = {
    Name = "${var.name_prefix}-private-redis-subnet-${count.index}"
  }
}



# define ${var.az_count} private subnets To SQL Server (one for each AZ)
resource "aws_subnet" "private_sql_subnet" {
  count                   = "${var.az_count}"
  cidr_block              = "${cidrsubnet(aws_vpc.main_network.cidr_block, 8, count.index)}"
  availability_zone       = "${data.aws_availability_zones.available_azs.names[count.index]}"
  vpc_id                  = "${aws_vpc.main_network.id}"
  map_public_ip_on_launch = false
  tags = {
    Name = "${var.name_prefix}-private-sql-subnet-${count.index}"
  }
}
...
# associate route tables with ECS private subnets
...
# associate route tables with redis private subnets
resource "aws_route_table_association" "private_route_table_association" {
  count          = "${var.az_count}"
  subnet_id      = "${element(aws_subnet.private_redis_subnet.*.id, count.index)}"
  route_table_id = "${element(aws_route_table.private_route_table.*.id, count.index)}"
}

# associate route tables with sql server private subnets
resource "aws_route_table_association" "private_route_table_association" {
  count          = "${var.az_count}"
  subnet_id      = "${element(aws_subnet.private_sql_subnet.*.id, count.index)}"
  route_table_id = "${element(aws_route_table.private_route_table.*.id, count.index)}"
}